...
export function RegisterIgg(){
    ...
    async function HandleRegister(data){
       const video = dataRoad.videos?.filter((item) => item.id === Number(data.id))
       const token = window.localStorage.getItem("encibraapptoken-v2");
        console.log(video[0].id)
       if(video){
        try {
            const response = await api.put( `/road/${dataRoad.id}/iggs/null/create`, 
            {
                ...
                <label htmlFor="">
                    ...
                    >
                        ...
                        {dataRoad.videos?.map((item) =>{
                            ...
                            return(
                                <option key={item.id} className="px-4 py-2" value={item.id}><p> {item.stretch}</p>  <p>{dataUTC}</p></option>
                            )
                        })}
                    </select>
                </label>
